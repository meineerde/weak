...
# That way, all values can be garbage collected and silently removed from the
# set unless they are still referenced from some other live object.
#
# WeakSet uses `ObjectSpace::WeakMap` as storage, so you must note the following
# points:
#
# - Equality of elements is determined strictly by their object identity instead
#   of `Object#eql?` or `Object#hash` as the Set does by default.
...
#   not preserved.
#
# Note that WeakSet is not inherently thread-safe. When accessing a WeakSet
# from multiple threads or fibers, you MUST use a mutex or another locking
# mechanism.
#
# ## Implementation Details
#
# The various Ruby implementations and versions show quite diverse behavior in
# their respective `ObjectSpace::WeakMap` implementations. To provide a unified
# behavior on all implementations, we use different storage strategies:
#
# - Ruby (aka. MRI, aka. YARV) >= 3.3 has a WeakMap with weak keys and weak
#   values and the ability to delete elements from it. This allows a
#   straight-forward implementation in {WeakSet::WeakKeysWithDelete}.
# - Ruby (aka. MRI, aka. YARV) < 3.3 has a WeakMap with weak keys and weak
#   values but does not allow to directly delete entries. We emulate this with
#   special garbage-collectible values in {WeakSet::WeakKeys}.
...
#   `object_id` as a key. As these WeakMaps also do not allow to delete entries,
#   we emulate deletion with special garbage-collectible values as above. This
#   is implemented in {WeakSet::StrongKeys}.
# - JRuby < 9.4.6.0 has a similar WeakMap as newer JRuby versions with strong
#   keys and weak values. However generally in JRuby, Integer values (including
#   object_ids) can have multiple different object representations and are not
#   necessarily equal to each other when used as keys in a WeakMap. As a
#   workaround we use a more indirect implementation with a secondary
#   lookup table for the keys in {WeakSet::StrongSecondaryKeys}.
#
...
#   require "weak_set"
#
#   s1 = WeakSet[1, 2]                    #=> #<WeakSet: {1, 2}>
#   s2 = WeakSet.new [1, 2]               #=> #<WeakSet: {1, 2}>
#   s1 == s2                              #=> true
#   s1.add(:foo)                          #=> #<WeakSet: {1, 2, :foo}>
#   s1.merge([2, 6])                      #=> #<WeakSet: {1, 2, :foo, 6}>
#   s1.subset?(s2)                        #=> false
#   s2.subset?(s1)                        #=> true
class WeakSet
  include Enumerable
  ...
  #   Deletes the given object from `self` and returns `self` if it was present
  ...
  #   @note The order of elements on the returned `Array` is non-deterministic.
  ...
  # @return [WeakSet] a new weak set containing the given objects
  #
  # @example
  #     WeakSet[1, 2]                   # => #<WeakSet: {1, 2}>
  #     WeakSet[1, 2, 1]                # => #<WeakSet: {1, 2}>
  #     WeakSet[1, :c, :s]              # => #<WeakSet: {1, :c, :s}>
  def self.[](*ary)
    new(ary)
  end
  ...
  #   only return objects which are references somewhere else to avoid them being
  ...
  # @param enum (see #do_with_enum)
  # @return [WeakSet] a new weak set built by merging `self` and the elements of
  #   the given enumerable object.
  # @!macro _note_object_equality
  #
  # @example
  #     WeakSet[1, 2, 3] | WeakSet[2, 4, 5] #=> #<WeakSet: {1, 2, 3, 4, 5}>
  #     WeakSet[1, 3, :z] | (1..4)          #=> #<WeakSet: {1, 3, :z, 2, 4}>
  def |(enum)
    new_set = dup
    do_with_enum(enum) do |obj|
      new_set.add(obj)
  ...
  # @param enum (see #do_with_enum)
  # @return [WeakSet] a new weak set built by duplicating `self`, removing every
  #   element that appears in the given enumerable object from that.
  # @!macro _note_object_equality
  #
  # @example
  #     WeakSet[1, 3, 5] - WeakSet[1, 5]            #=> #<WeakSet: {3}>
  #     WeakSet['a', 'b', 'z'] - ['a', 'c']         #=> #<WeakSet: {"b", "z"}>
  def -(enum)
    dup.subtract(enum)
  end
  ...
  # @return [WeakSet] a new weak set containing elements common to `self` and
  #   the given enumerable object.
  # @!macro _note_object_equality
  #
  # @example
  ...
  #
  # @param other [WeakSet] a weak set to compare to `self`
  # @return [Bool] `true` if the `other` object is a weak set containing exactly
  #   the same elements as `self`, `false` otherwise
  #
  # @example
  #     WeakSet[1, 2] == WeakSet[2, 1]           #=> true
  ...
  # @param obj [Object] an object
  # @return [Object, nil] the provided `obj` if it is included in `self`, `nil`
  #   otherwise
  # @see #include?
  # @!macro _note_object_equality
  def [](obj)
  ...
  # @return [self, nil] `self` if the object was added, `nil` if it was part of
  #   the set already
  # @!macro _note_object_equality
  #
  # @example
  ...
  #   this is set toi a truethy value., we emit a warning
  # @return [WeakSet] a new WeakSet object containing the same elements as
  #   `self`
  def clone(freeze: false)
    warn("Can't freeze WeakSet") if freeze
  ...
  # value, and returns `self`. Returns an `Enumerator` if no block is given.
  #
  # @yield [element] calls the given block once with each element. If the block
  #   returns a truethy value, the element is deleted from the set
  ...
  # @return [self]
  def freeze
    warn("Can't freeze WeakSet")
    self
  end

  # @return [String]  a string containing a human-readable representation of the
  ...
  #
  # @example
  #     WeakSet[1, 2, 3].intersect? WeakSet[4, 5]   #=> false
  #     WeakSet[1, 2, 3].intersect? WeakSet[3, 4]   #=> true
  #     WeakSet[1, 2, 3].intersect? 4..5            #=> false
  #     WeakSet[1, 2, 3].intersect? [3, 4]          #=> true
  def intersect?(enum)
    case enum
    when WeakSet
      enum_ary = enum.to_a
  ...
  # @example
  #
  #     set = WeakSet[1, :c, :s]          #=> #<WeakSet: {1, :c, :s}>
  #     set.replace([1, 2])               #=> #<WeakSet: {1, 2}>
  #     set                               #=> #<WeakSet: {1, 2}>
  def replace(enum)
    cleared do
      do_with_enum(enum) do |obj|
  ...
  # @param enum [WeakSet, #each_entry #each] a {WeakSet} or an enumerable
  #   object
  # return [void]
  ...
  # @param enum [WeakSet, Enumerable, #each_entry, #each] a {WeakSet}, or an
  #   Enumerable object, e.g. an `Array` or `Set`, or an object which responds
  #   to `each_entry` or `each`
